              ...
              <SheetContent side="right" className="w-full max-w-sm p-0">
                ...
                <div className="flex max-h-[calc(100dvh-57px)] flex-col overflow-y-auto px-4 py-4">
                  {user && userProfile && (
                    <div className="mb-6">
                      ...
                      <div className="space-y-1">
                        <Link href={dashboardUrl} onClick={() => setIsSheetOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent/50">
                          <LayoutDashboard className="h-4 w-4" />
                          Dashboard
                        </Link>
                        {isSeller && (
                          <Link href="/dashboard/listings" onClick={() => setIsSheetOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent/50">
                            <PlusCircle className="h-4 w-4" />
                            My Listings
                          </Link>
                        )}
                        <Link href="/messages" onClick={() => setIsSheetOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent/50">
                          <MessageSquare className="h-4 w-4" />
                          Messages
                        </Link>
                        <Link href="/favorites" onClick={() => setIsSheetOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent/50">
                          <Heart className="h-4 w-4" />
                          Saved Properties
                        </Link>
                        <Link href="/profile" onClick={() => setIsSheetOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent/50">
                  ...
                  <div className="mt-6 border-t px-2 pt-4">
                    {user && userProfile ? (
                      <Button variant="outline" onClick={() => { handleLogout(); setIsSheetOpen(false); }} className="w-full">
                        <LogOut className="mr-2 h-4 w-4" />
                        Log out
                      </Button>
                    ) : (
